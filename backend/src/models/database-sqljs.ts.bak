...
export async function initializeDatabase() {
  try {
    if (usePostgres) {
      ...
      // Indexes
      await pgClient.query('CREATE INDEX IF NOT EXISTS idx_messages_conversation ON messages(conversation_id)');
      await pgClient.query('CREATE INDEX IF NOT EXISTS idx_users_location ON users(latitude, longitude)');
    ...
    db.run(`CREATE INDEX IF NOT EXISTS idx_users_location ON users(latitude, longitude)`);

    // Save to disk
    saveDatabase();
    console.log('✅ Database schema initialized successfully');
    
    // Start auto-save after database is initialized
...
// Save database to disk (only for sql.js mode)
...
// Save on exit - but don't cause early exit
process.on('beforeExit', saveDatabase);

export function getDb(): Database {
  if (usePostgres) {
    throw new Error('SQL.js database not active when using Postgres. Use Postgres client instead.');
  }
  if (!db) {
