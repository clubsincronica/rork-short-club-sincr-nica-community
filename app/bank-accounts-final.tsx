...
let globalState = {
  ...
  editingAccount: null as BankAccount | null,
  formData: {
    bankName: '',
    accountNumber: '',
    accountHolderName: '',
...
export default function BankAccountsManagement() {
  ...
  const handleSave = () => {
    if (!globalState.formData.bankName || !globalState.formData.accountNumber || !globalState.formData.accountHolderName) {
      Alert.alert('Error', 'Por favor completa todos los campos obligatorios');
      return;
    }

    const currentTotal = (currentUser?.bankAccounts || [])
      .filter((acc: BankAccount) => globalState.editingAccount ? acc.id !== globalState.editingAccount.id : true)
      .reduce((sum: number, acc: BankAccount) => sum + acc.percentage, 0);

    if (currentTotal + globalState.formData.percentage > 100) {
      Alert.alert('Error', 'El total de porcentajes no puede exceder 100%');
      return;
    }

    if (globalState.editingAccount) {
      updateBankAccount({ accountId: globalState.editingAccount.id, updates: { ...globalState.formData, accountType: 'checking' as const, currency: 'EUR', country: 'ES', isVerified: false, isActive: true } });
    } else {
      addBankAccount({ ...globalState.formData, accountType: 'checking' as const, currency: 'EUR', country: 'ES', isVerified: false, isActive: true });
    }
  ...
  const isFormValid = globalState.formData.bankName && globalState.formData.accountNumber && globalState.formData.accountHolderName;
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {/* Add/Edit Modal */}
      <Modal
        visible={globalState.isAddModalVisible}
        transparent
        animationType="slide"
        onRequestClose={() => updateState({ isAddModalVisible: false })}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {globalState.editingAccount ? 'Editar Cuenta' : 'Nueva Cuenta Bancaria'}
              </Text>
              <TouchableOpacity 
                onPress={() => updateState({ isAddModalVisible: false })}
            ...
            <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  style={styles.input}
                  value={globalState.formData.bankName}
                  onChangeText={(text: string) => updateFormData('bankName', text)}
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  style={styles.input}
                  value={globalState.formData.accountNumber}
                  onChangeText={(text: string) => updateFormData('accountNumber', text)}
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  style={styles.input}
                  value={globalState.formData.accountHolderName}
                  onChangeText={(text: string) => updateFormData('accountHolderName', text)}
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  style={styles.input}
                  value={globalState.formData.percentage.toString()}
                  onChangeText={(text: string) => updateFormData('percentage', parseFloat(text) || 0)}
                  placeholder="50"
                  placeholderTextColor={Colors.textLight}
                  keyboardType="numeric"
                />
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  style={styles.input}
                  value={globalState.formData.nickname}
                  onChangeText={(text: string) => updateFormData('nickname', text)}
            ...
            <View style={styles.modalFooter}>
              ...
              >
                <Text style={styles.saveButtonText}>
                  {globalState.editingAccount ? 'Actualizar' : 'Guardar'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
